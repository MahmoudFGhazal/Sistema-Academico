...
void CadastrarUsuario(const char *nomeArquivo)
{
    ...
    while(1){
        printf("Digite a senha até 9 caracteres: ");
        // Lê a senha do usuário, com no máximo 9 caracteres
        scanf("%s", novoUsuario.senha);
        if(strlen(novoUsuario.senha) > 10 ){
...
void Professor(membroAcademico professor)
{
    ...
    while(1){
        ...
        char ch = getchar();
        //isspace: ver se o getchar é vazio
        if(!isspace(ch)){
        ...
        {
            printf("Erro: Você não está registrado como professor da matéria ou matéria não existe: %s.\n", nomeMateria);
            fflush(stdin);
            getchar();
            system("cls");
            continue;
        ...
        do
        {
            ...
            case '4':
                ...
                {
                    // Pergunta se deseja realmente sair
                    printf("Deseja escolher outra materia?\n1- Sim\n2- Nao\nInsira sua opcao: ");
                    scanf("%s", escolha);
                    fflush(stdin);
                    // Verifica o comprimento da escolha de saída
                    if (strlen(escolha) > 1)
...
void Aluno(membroAcademico aluno)
{
    ...
    while(1){
        ...
        fgets(nomeMateria, 100, stdin);
        printf("%s", nomeMateria);
        nomeMateria[strcspn(nomeMateria, "\n")] = '\0';


        if (!AlunoExisteNaMateria(nomeMateria, aluno.nome))
        {
            printf("Erro: Você não está registrado como aluno da matéria ou matéria não existe: %s.\n", nomeMateria);
            fflush(stdin);
            getchar();
            system("cls");
            continue;
        ...
        do
        {
            ...
            switch (opcao[0])
            {
                ...
                case '3':
                    ...
                    {
                        // Pergunta se deseja realmente sair
                        printf("Deseja ver mais alguma materia?\n1- Sim\n2- Nao\n");
                        scanf("%s", escolha);
                        fflush(stdin);
                        // Verifica o comprimento da escolha de saída
                        if (strlen(escolha) > 1)
